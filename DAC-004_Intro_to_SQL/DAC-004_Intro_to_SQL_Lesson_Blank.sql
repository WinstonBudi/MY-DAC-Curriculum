...
SELECT
	...
	ROUND (AVG(vacationhours)) AS rounded_Vacay_hours,
...
-- If it is a one-to-one relationship, the chances of having duplicates are unlikely.
-- However, if it is a one-to-one relationship, there could be a chance for duplicates to be present.
...
SELECT
	...
	CURRENT_DATE + INTERVAL '10 days' AS add_month
FROM sales.salesorderheader
WHERE territoryid = 1
